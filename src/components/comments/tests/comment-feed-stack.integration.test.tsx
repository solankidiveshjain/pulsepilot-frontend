...
import { mockReplies, mockUsers, mockComments as rawMockComments } from "@/lib/mock-data"; // Correct import path
import type { Comment } from "@/types";
import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
import { act, fireEvent, render, screen, waitFor } from "@testing-library/react"; // Added act
...
describe("CommentFeed Stack Integration Tests", () => {
  ...
  describe("Keyboard Navigation", () => {
    it("should select next/previous comment on ArrowDown/ArrowUp", async () => {
      ...
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        setSelectedComment: setSelectedCommentMock,
      });
  ...
  describe("CommentCard Interactions via CommentsFeedContainer", () => {
    it("should call toggleCommentSelection when checkbox is clicked", async () => {
      ...
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        toggleCommentSelection: toggleCommentSelectionMock,
      });
      ...
      const _checkboxSelectionButton = screen
        .getByText(mockComments[0].author.name) // Find an element in the card
      ...
      const checkboxInAvatar = firstInnerDiv?.querySelector("button"); // First avatar checkbox button

      if (checkboxInAvatar) {
        await user.click(checkboxInAvatar);
        expect(toggleCommentSelectionMock).toHaveBeenCalledWith(mockComments[0].id);
      } else {
        throw new Error(
          "Could not find the selection checkbox button with current selectors. Consider adding a data-testid."
        );
      }
    });
    ...
    it('should call onAction with "archive" when archive action is clicked in dropdown', async () => {
      ...
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        archiveComments: archiveCommentsMock,
      });
    ...
    it('should call toggleExpandReply when "View replies" is clicked', async () => {
      ...
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        toggleExpandReply: toggleExpandReplyMock,
      });
      ...
      const viewRepliesButton = screen.getByRole("button", {
        name: (content, element) => content.startsWith(String(commentWithReplies.replies)),
      });
    ...
    it('should call toggleExpandComment when "See more" is clicked for long comments', async () => {
      ...
      const longComment = {
        ...mockComments[0],
        text: "This is a very long comment that definitely exceeds the one hundred and eighty character limit for truncation and therefore should display a see more button to allow the user to expand it and read the full content.",
      };
      mockUseCommentsImplementation({ items: [longComment] });
      const toggleExpandCommentMock = vi.fn();
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        toggleExpandComment: toggleExpandCommentMock,
      });
    ...
    it('should call onAction with "save" when save action is clicked', async () => {
      ...
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        saveCommentForLater: saveCommentForLaterMock,
      });
    ...
    it('should call onAction with "important" when mark as important action is clicked', async () => {
      ...
      mockedUseCommentsActions.mockReturnValue({
        ...mockDefaultActions,
        markCommentAsImportant: markCommentAsImportantMock,
      });
